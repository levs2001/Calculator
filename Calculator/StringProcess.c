...
int CheckString(char* string, int size)
{
	...
	for (; index < size - 1 && string[index] >= 0 && isspace(string[index]) != 0; index++) {};
	...
	for (int i = index; i < size - 1; i++) {
		if (string[i] < 0 || !(isspace(string[i])!=0 || isalnum(string[i])!=0 || string[i] == '(' || string[i] == ')' || string[i] == '+' || string[i] == '-' || string[i] == '*' || string[i] == '/' || string[i] == '^' || string[i] == '.' || string[i] == ',' || string[i] == ';' || string[i] == '=')) {
			string[size - 1] = '\0';
			str_type = ERR_STRING;
			return str_type;
		}
