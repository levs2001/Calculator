...
int FindPriority(char* string, int* _size_string, int* _priority_index, double* numbers) {
	...
	*_priority_index = priority_index;
	...
	//Делаем проверку на логарифм здесь
	if (priority_index == -1) {
		//Здесь проверяем есть ли запятая, если есть, то проверяем есть ли перед этим лог
		//Если да, то делаем отдельную операцию для логарифма
		for (int i = begin; i < end; i++) {
			if (string[i] == ',') {
				priority_index = i;
				break;
...
int FindLog(char* string, int size_string, int* _priority_index) {
	...
	//проверяем что перед , и после, если log(0,0), то всё ок вызываем
